...
public class AlienController : MonoBehaviour
{
    ...
    private Vector3 target; // 아바타의 상체
...
        }
        ...
        {
            /*
            구르기가 중첩 실행되지 않기 위해 bool isDodging값을
            애니메이션이 끝날 때 false로 설정하고 false일때 
            스페이스 누를 시 1번 true로 바꿈
            */
    ...
    public void Victory(Vector3 nowPos)
    {
        ...
        Quaternion rot = Quaternion.LookRotation(mainCamera.transform.position - transform.position);
        transform.rotation = Quaternion.Slerp(transform.rotation, rot, Time.deltaTime * 0.9f);
    }
            ...
            if (bulletStack <= 5)
            {
                ...
                if (shootingStack >= 0.2f)
                {
                    ...
                    bullets = Instantiate(bullet,
                              ...
                              spine.transform.rotation) as GameObject;
