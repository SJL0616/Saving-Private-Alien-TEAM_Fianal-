...
public class EnemyDetectionController : MonoBehaviour
{
   ...
   // public GameObject bullet;
    ...
    void FindTargets()
    {
        ...
        if(targetsInCombatRadius.Length == 0){//즉각 사격 범위에 없다면 기본 범위에서 탐색
        ...
        for (int i =0; i< targetsInViewRadius.Length; i++)
        {
            ...
            { 
                ...
                if (Physics.Raycast(transform.position,dirToTarget,out hit,dstToTarget))
                {
                    ...
                    if(!isShooting){//플레이어가 시야각에 있지만 공격상태가 아닌 경우 약 2초간 응시.
                        move.walk(false);
                        isLookOut = true;
                        Invoke("Identifying",1.0f);
                    }
                    if(isImmediateCombat){//즉각 사격 상태일시
                        //move.shoot(isFreeToFire);
                    ...
                    if(hit.transform.gameObject.layer != 3 ){ //사이에 엄폐물 존재시
                        if(!move.isDetouring && !move.isBouncing && isShooting ){
                            rayStack += 1;
                            if(rayStack >= 4){ //일정 시간 후에 플레이어를 향해 이동 시작
                                move.goToTarget(target);
                                rayStack = 0;
                            }
                        }  
