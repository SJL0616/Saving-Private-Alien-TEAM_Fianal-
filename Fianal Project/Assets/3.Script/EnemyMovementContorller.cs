...
public class EnemyMovementContorller : MonoBehaviour
{
    ...
    public void shoot(bool inputBool){ 
        if(cam.GetComponent<CamAudioController>().currentBgm == 1){
            cam.GetComponent<CamAudioController>().PlaySound("Combat");
            cam.GetComponent<CamAudioController>().currentBgm +=1;
        }
        if(inputBool){
    ...
    void OnTriggerEnter(Collider coll){ //패트롤에 필요한 WayPoint 번호를 갱신
        if(coll.tag == "WayPoint") //WayPoint 태그를 단 gameObject와 닿을 시 실행됨
        {
            nextIdx = (++nextIdx >= wayPoints.Length) ? 1 : nextIdx;
        }
    }
    IEnumerator startPatrol(){ // 패트롤을 실행시키는 코루틴 메서드
        isPatrolling = true;
        while (true)
        {
           yield return null;
    ...
    void Patrolling(){ //정찰 지점으로 회전시키는 메서드
    ...
    void Update()
    {
        if(agent.velocity.sqrMagnitude >= 0.5f  && agent.remainingDistance <= 4.5f){
                walk(false);
                Crouch(true);
                isDetouring = false;
        }
